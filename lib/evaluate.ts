// Shared evaluation logic — used by both Express (local dev) and Vercel (production)
...
export interface Recall {
  id: string; component: string; summary: string; remedy: string;
}
...
// ── Brand-specific annual depreciation rates ──────────────────────────────────
...
function buildStrategy(
  ...
  if (cond <= 2) {
    s1 = `This ${year} ${make} ${model}'s below-average condition (${cond}/5) will accelerate depreciation beyond the typical ${rateStr} annual rate for this brand.`;
    ...
    s1 = `At ${miles.toLocaleString()} miles, this ${year} ${make} ${model} has entered the bracket where maintenance costs often outpace residual-value gains.`;
    s2 = "Listing at current market price is recommended before high-mileage penalties deepen; buyer interest narrows significantly past 150K.";
  } else if (miles >= 80000) {
    s1 = `This ${year} ${make} ${model} is projected to shed ~${depPct}% over the next 12 months, driven by ${make}'s ${rateStr} annual rate and above-average mileage.`;
    ...
    s1 = `High-value vehicles like this ${year} ${make} ${model} face compressed demand in the used market, amplifying the projected ${depPct}% 12-month decline.`;
    s2 = cond >= 4
      ? "Strong condition supports holding — reasonable if mileage stays low and no new competing model is released this cycle."
      : "Selling at a competitive listing price now will outperform waiting as luxury depreciation steepens with age.";
    ...
    s2 = "Holding is viable for 12–18 more months; listing before the next model-year release (typically Q3) maximizes resale leverage.";
  } else {
    s1 = `Based on ${make}'s ${rateStr} annual depreciation, this ${year} ${make} ${model} is on track to lose ~${depPct}% — roughly $${Math.round(value * parseFloat(depPct) / 100).toLocaleString()} — over the next 12 months.`;
...
export async function evaluate(body: {
  vin: string; mileage?: string; zip?: string; condition?: string;
}): Promise<EvalResult> {
  const { vin, mileage, zip, condition } = body;

  if (!vin || vin.trim().length !== 17) {
    const err = new Error("Please enter a valid 17-character VIN.");
    (err as any).statusCode = 400;
    throw err;
  }

  // 1. Decode VIN via NHTSA VPIC (free, no key)
  ...
  const miles = parseInt(mileage ?? "0") || 0;
  const cond = Math.max(1, Math.min(5, parseInt(condition ?? "3") || 3));
